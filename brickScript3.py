...
class brick:
	...
	def __init__(self, pos, dimension):
		...
		rs.SurfaceIsocurveDensity(self.id, 0)
		self.vertex = vert
	...
	def __init__(self, curveID, lNum, ht):
		...
		while c < courseNum:
			...
			while layer < len(curveList):
				...
				p = dom[0]
				pShiftLayer = ((brickDim[0]/4)+(self.mortarThickness/4))/distRatio(curID, p)
				if layerEven:
					p += pShiftLayer
				
				if isEven:
					pShift = ((brickDim[0]/4)+(self.mortarThickness/4))/distRatio(curID, p)
					p += pShift
